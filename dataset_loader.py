...
def load_mat(args):
    ...
    if args.dataset == 'Scene15':
        mat = sio.loadmat(os.path.join(GLOBAL_PATH, 'Scene15.mat'))
        X = mat['X'][0]
        ...
        mat = sio.loadmat(os.path.join(GLOBAL_PATH, 'Fashion.mat'))
        ...
        print('number of Fashion datset = ', len(label_y))
    ...
    elif args.dataset in ['bbc', 'mini_news','ag_news', 'news']:
        data = sio.loadmat(GLOBAL_PATH + args.dataset +'.mat')
        label_y = data['y'].reshape(-1)
        print(label_y.shape)
        data_X = [data['x1']*1.0,data['x0']*1.0]
        for x in data_X:
            print(x.shape)
        print(label_y.shape)

    elif args.dataset == 'ag_news_train':
        path = GLOBAL_PATH + 'ag_news_train.mat'
        import h5py
        # import hdf5storage
        # data = hdf5storage.loadmat(path)
        data=h5py.File(path) 
        data_X = [np.array(data['x0']).T*1.0, np.array(data['x1']).T*1.0]
        data_y=sio.loadmat(GLOBAL_PATH + 'ag_news_train_y.mat') 
        label_y = np.array(data_y['d1']).reshape(-1)
        for x in data_X:
            print(x.shape)
        print(label_y.shape)
    
    elif args.dataset == 'chinese_news':
        data = sio.loadmat(GLOBAL_PATH + 'chinese_news_3.mat')
        print(data.keys())
        label_y = data['y'].reshape(-1)
        data_X = [data['x0']*1.0,data['x1']*1.0]
    
    elif args.dataset == 'AMiner-4v':
        data = sio.loadmat(GLOBAL_PATH + 'AMiner_4v.mat')
        label_y = data['y'].reshape(-1)
        print(label_y.shape)
        data_X = [data['x0'],data['x1'],data['x2'],data['x3']]
